...
# -*- coding: utf-8 -*-
"""
DSCI 553 Yelp Competition - Final Hybrid (Option A: Safe & Fast, RDD-only)

Components:
  - Global mean + user/item biases
  - Item-based CF on residuals (Pearson with shrinkage)
  - MF (biased SVD via SGD + momentum) + implicit user vectors (SVD++-lite)
  - XGBoost on engineered side + PCA features
  - PCA on:
      * user compliment counts (11 -> 3)
      * business categories (BoW 60 -> 6)
      * business city (one-hot 120 -> 5)
      * business state (one-hot 25 -> 4)
  - Linear ridge blending of CF / MF / XGBoost

Run:
  spark-submit competition.py <folder_path> <test_file> <output_file>
"""
...
def pearson_with_shrink(pairs):
    ...
    if d1 == 0 or d2 == 0:
        return None
    corr = num / (d1 * d2)
    return corr * (n / (n + 5.0))  # shrinkage


def cf_predict(u, b, sim_bc, user_bc, g, bu_bc, bi_bc,
               thr=0.05, k=120, alpha=1.1, resid_cap=2.4):
...
def train_mf(u_idx, i_idx, ratings, n_users, n_items,
             k=80, epochs=10, lr=0.006, reg=0.05, g=3.5, momentum=0.8):
    rng = np.random.RandomState(42)
    P = 0.1 * rng.randn(n_users, k).astype(np.float32)
    Q = 0.1 * rng.randn(n_items, k).astype(np.float32)
...
def extract_side_features(folder, sc):
    ...
    if os.path.exists(user_path):
        ...
        if user_rows:
            ...
            print("DEBUG user compliment PCA: input=%s, components=%d"
                  % (comp_mat.shape, n_pc_user))
    ...
    if os.path.exists(biz_path):
        ...
        print("DEBUG biz category PCA: input=%s, components=%d" % (cat_mat.shape, n_cat_pc))
        print("DEBUG biz city PCA: input=%s, components=%d" % (city_mat.shape, n_city_pc))
        print("DEBUG biz state PCA: input=%s, components=%d" % (state_mat.shape, n_state_pc))
    ...
    print(
        "DEBUG side features loaded: users=%d, biz=%d, checkins=%d, tips=%d, photos=%d"
        % (len(users_feat), len(biz_feat), len(check_map), len(tip_map), len(photo_map))
    )
...
def run_analytics(user_business_rdd, business_user_rdd, test_rdd, train_count):
    ...
    print(
        "DEBUG stats: ratings=%d users=%d biz=%d density=%.8f"
        % (train_count, n_users, n_biz, density)
    )
    print("DEBUG rating dist:", rating_hist)
    print(
        "DEBUG user count min/med/mean/max: %d/%.1f/%.2f/%d"
        % (min(user_vals), median(user_vals), sum(user_vals) / n_users, max(user_vals))
    )
    print(
        "DEBUG biz count min/med/mean/max: %d/%.1f/%.2f/%d"
        % (min(biz_vals), median(biz_vals), sum(biz_vals) / n_biz, max(biz_vals))
    )
...
def main():
    ...
    conf = SparkConf().setAppName("DSCI553_Hybrid_OptionA_PCAUserBiz")
    ...
    print("DEBUG global mean:", round(g, 4))
    ...
    print("DEBUG #users=%d #biz=%d" % (len(user_dict), len(biz_dict)))

    # Biases
    bu, bi = compute_biases(user_dict, biz_dict, g, lam=9.0, iters=4)

    # CF similarities (no Slope-One to save memory)
    ...
    print("DEBUG similarity pairs:", len(sim_map))
    ...
    print("DEBUG number of features:", X_train.shape[1])
    print("DEBUG feature order (index: name):")
    for idx, name in enumerate(FEATURE_NAMES):
        print("  %2d: %s" % (idx, name))
    if X_train.shape[0] > 0:
        print("DEBUG sample feature row[0]:", X_train[0].tolist())
    ...
    print("Building implicit user vectors (SVD++-lite)...")
    ...
    if has_truth:
        ...
        print(
            "DEBUG RMSE pre-blend bias=%.4f cf=%.4f mf=%.4f xgb=%.4f"
            % (rm_bias, rm_cf, rm_mf, rm_xgb)
        )

        # Ridge-regularized linear blend:
        # final = w_cf*cf + w_mf*mf + w_xgb*xgb + c0
        Xb = []
        yb = []
        for cf_p, mf_p, xg_p, t in zip(cf_pred, mf_pred, xgb_pred, truths):
            if t is None:
                continue
            Xb.append([cf_p, mf_p, xg_p, 1.0])
            yb.append(t)
        ...
        try:
            ...
            XT = Xb.T
            A = XT.dot(Xb)
            for i in range(3):  # ridge on first 3 weights
                A[i, i] += lam
            b_vec = XT.dot(yb)
            coef = np.linalg.solve(A, b_vec)
            w_cf, w_mf, w_xgb, c0 = [float(x) for x in coef]
        except Exception:
            w_cf, w_mf, w_xgb, c0 = 0.13, 0.05, 0.87, -0.25

        # Clamp weights to avoid pathological solutions
        w_xgb = max(0.40, min(1.20, w_xgb))
        w_cf = max(-0.10, min(0.40, w_cf))
        w_mf = max(-0.10, min(0.40, w_mf))

        print(
            "DEBUG blend weights CF=%.3f MF=%.3f XGB=%.3f c=%.3f"
            % (w_cf, w_mf, w_xgb, c0)
        )

        for cf_p, mf_p, xg_p in zip(cf_pred, mf_pred, xgb_pred):
            val = w_cf * cf_p + w_mf * mf_p + w_xgb * xg_p + c0
            preds_final.append(clamp_1_5(val))

        rm_final = rmse(preds_final, truths)
        print("DEBUG RMSE blend=%.4f" % rm_final)
    else:
        # Default weights if no ground truth in test file
        w_cf, w_mf, w_xgb, c0 = 0.13, 0.05, 0.87, -0.25
        print(
            "DEBUG blend (no truths) CF=%.2f MF=%.2f XGB=%.2f"
            % (w_cf, w_mf, w_xgb)
        )
        for cf_p, mf_p, xg_p in zip(cf_pred, mf_pred, xgb_pred):
            val = w_cf * cf_p + w_mf * mf_p + w_xgb * xg_p + c0
